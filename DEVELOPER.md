...
> [!NOTE]
> On current firmware, `configs/1` is expected to fail the main-heap check.
> That keeps CI red until the always-active path has enough headroom (or the
> fixture is temporarily removed). `configs/2` should pass.
