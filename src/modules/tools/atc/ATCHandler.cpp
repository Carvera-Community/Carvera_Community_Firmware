...
bool ATCHandler::finalize_tool_dia_measurement(const char* source_tag)
{
	...
	if (!this->tool_dia_probe_start_valid) {
		THEKERNEL->eeprom_data->reserve = 0.0f;
	...
	if (diameter > 0.0f) {
		...
		THEKERNEL->write_eeprom_data();
		THEKERNEL->streams->printf("Measured tool diameter [%.3f] saved to TOOL_DIA (M491.2-certified, %s)\n", diameter, src);
		this->tool_dia_probe_start_valid = false;
		return true;
	}
...
void ATCHandler::load_custom_tool_slots() {
    ...
    char buf[132];
    while (fwfs::fgets(buf, sizeof(buf)-1, fp) != NULL) {
        string line(buf);
...
void ATCHandler::save_custom_tool_slots_to_file() {
    ...
    fwfs::fprintf(fp, "# Custom tool slot configuration\n");
    fwfs::fprintf(fp, "# Format: tool_num x y [z]\n");
    fwfs::fprintf(fp, "# Example: 10 -3.526 -61.643 -119.1\n");
    fwfs::fprintf(fp, "#          (Z is optional, uses previous tool's Z if not provided)\n");
    fwfs::fprintf(fp, "# This file is automatically generated. Do not edit manually.\n\n");
    
    // Write all valid tool slots (one line per tool)
    bool wrote_any = false;
    for (const auto& tool : atc_tools) {
        if (tool.is_valid()) {
            fwfs::fprintf(fp, "%d %.3f %.3f %.3f\n", tool.num, tool.get_mx_mm(), tool.get_my_mm(), tool.get_mz_mm());
            wrote_any = true;
        }
    }
...
void ATCHandler::fill_calibrate_probe_anchor_scripts(bool invert_probe){
	...
	snprintf(buff, sizeof(buff), "G91 G53 G0 X%.3f Y%.3f", this->anchor1_x + this->anchor2_offset_x - this->anchor_width/2, this->anchor1_y + this->anchor2_offset_y + 15);
	this->script_queue.push(buff);
	...
	if (!gcode->has_letter('P')){
		...
		switch ((int)gcode->get_value('P')){
			...
			case 5:
				...
				//M469.7 A#120 B#119 C#118 Z%.3f P5
...
void ATCHandler::calibrate_anchor1(Gcode *gcode) //M469.1
{
	...
	snprintf(buff, sizeof(buff), "M469.7 X%.3f Y%.3f P1", this->anchor1_x , this->anchor1_y);
	this->script_queue.push(buff);
}
...
void ATCHandler::calibrate_anchor2(Gcode *gcode)//M469.2
{
	...
	snprintf(buff, sizeof(buff), "M469.7 X%.3f Y%.3f P2", this->anchor2_offset_x , this->anchor2_offset_y);
	this->script_queue.push(buff);
}
...
void ATCHandler::calibrate_a_axis_headstock(Gcode *gcode)//M469.4
{
	...
	snprintf(buff, sizeof(buff), "M469.7 Y%.3f P4" , this->rotation_offset_y);
	this->script_queue.push(buff);
...
void ATCHandler::calibrate_a_axis_height(Gcode *gcode) //M469.5
{
	...
	//execute calibration with specific values
	snprintf(buff, sizeof(buff), "M469.7 A#120 B#119 C#118 D#117 E#116 R%.3f Z%.3f P5" , pin_diameter, this->rotation_offset_z);
	this->script_queue.push(buff);
}
...
void ATCHandler::fill_change_scripts(int new_tool, bool clear_z, int old_tool = -1, bool wait_after_empty = false, uint8_t colletIndex = 0, float custom_TLO = NAN) {
	...
	if (THEKERNEL->factory_set->FuncSetting & (1<<2)){
		...
		if (!isnan(custom_TLO)) {
			//print status
			snprintf(buff, sizeof(buff), "Tool is now installed and TLO set as %.3f." , custom_TLO );
			this->script_queue.push(buff);
...
void ATCHandler::fill_cali_scripts(bool is_probe, bool clear_z, int repeat_count, bool measure_diameter) {
	...
	for(int i = 1; i <= repeat_count; i++){
		...
		if(i == repeat_count){
			...
			// Tool diameter measurement is only enabled for the explicit M491.2 path.
    ...
    if (gcode->has_m) {
    	...
    	if (gcode->m == 6 && gcode->has_letter('T')) {
			...
			if (gcode->subcode == 1) {
				...
				if (gcode->has_letter('H')) {
					...
					if (tolerance < 0.02) {
						...
						THEKERNEL->set_halt_reason(CALIBRATE_FAIL);
						THEKERNEL->call_event(ON_HALT, nullptr);
				bool is_tool_break_check = gcode->has_letter('P');
				
				if (is_tool_break_check) {
					float tlo = 0;
					float tolerance = 0.1;
					if (gcode->has_letter('H')) {
		    				tolerance = gcode->get_value('H');
					...
					if (active_tool == 0 || active_tool >= 999990) {
						THEKERNEL->streams->printf("ALARM: M491.2 cannot run with probe tool installed. Please select a regular tool.\n");
						THEKERNEL->call_event(ON_HALT, nullptr);
						THEKERNEL->set_halt_reason(PROBE_FAIL);
						return;
					}
				...
				if (fabs(tlo - new_tlo) > tolerance) {
					...
					THEKERNEL->streams->printf("M491.2 Probe-Safe Diameter Measurement\n");
					// Invalidate previous M491.2 certification until this run completes successfully.
					THEKERNEL->eeprom_data->reserve = 0.0f;
					THEKERNEL->write_eeprom_data();
					THEROBOT->push_state();
					THEROBOT->get_axis_position(last_pos, 3);
			...
			if (gcode->subcode == 0 || gcode->subcode == 1) {
				...
				if (gcode->has_letter('D')) {
					float dia = gcode->get_value('D');
					if (dia > 0.0f) {
						THEKERNEL->eeprom_data->TOOL_DIA = dia;
						THEKERNEL->eeprom_data->reserve = 0.0f;
						THEKERNEL->write_eeprom_data();
						THEKERNEL->streams->printf("Tool diameter set to %.3fmm (manual, not M491.2-certified)\n", dia);
					} else {
				...
				if (gcode->has_letter('T')) {
					...
					THEROBOT->set_tool_not_calibrated(true);
					// Tool changes invalidate previously measured cutter diameter provenance.
					THEKERNEL->eeprom_data->reserve = 0.0f;
				...
				THEKERNEL->streams->printf("TOOL_DIA source: %s\n", tool_dia_from_m4912(THEKERNEL->eeprom_data) ? "M491.2-certified" : "manual/unknown");
    ...
    if (this->atc_status != NONE) {
        ...
        if (ok) {
            ...
            if (this->playing_file && !playing) {
				...
				this->atc_status = NONE;
				// Clear TLO calibration flag and M491.2 mode to re-enable 3D probe crash detection
		...
		if (this->tool_dia_probe_start_valid) {
			THEKERNEL->streams->printf("DEBUG: Finalizing pending M491.2 diameter at ATC completion\n");
