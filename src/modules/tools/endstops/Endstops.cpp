...
bool Endstops::load_old_config()
{
    uint16_t const checksums[][NDEFNS] = {
        ...
        ENDSTOP_CHECKSUMS("epsilon")  // B    // Z
    };

    bool limit_enabled = false;
    if(THEKERNEL->factory_set->FuncSetting & ((1<<0)|(1<<1)))
    {
		for (int i = X_AXIS; i <= B_AXIS; ++i) { // X_AXIS to B_AXIS
	        homing_info_t hinfo;
	
	        // init homing struct
	        hinfo.home_offset = 0;
	        hinfo.homed = false;
	        hinfo.axis =	i >Z_AXIS ? 'A'+i-3 : 'X'+i;
	        hinfo.axis_index = i;
...
void Endstops::on_idle(void *argument)
{
    ...
    for(auto& i : endstops) {
    	if(THEKERNEL->factory_set->FuncSetting & ((1<<0)|(1<<1)))
    	{
	    	if(i->limit_enable && STEPPER[i->axis_index]->is_moving() && ((i->axis!='A') && (i->axis!='B')) ) {
	            // check min and max endstops
	            if(debounced_get(&i->pin)) {
	                // endstop triggered
	                if(!THEKERNEL->is_grbl_mode()) {
...
void Endstops::on_get_public_data(void* argument)
{
    ...
    if(pdr->second_element_is(trim_checksum)) {
        ...
        *cover_state = this->cover_endstop_pin.get();
        ...
        data[0] = homing_axis[A_AXIS].pin_info->pin.get();
