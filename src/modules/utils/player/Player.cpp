...
static uint8_t fbuff[4096] LOCATED_IN_AHBSRAM;
...
int Player::read_byte(StreamOutput *stream, unsigned int timeout_ms)
{
...
int Player::read_bytes(StreamOutput *stream, char **buf, int size, unsigned int timeout_ms)
{
	uint32_t tick_us = us_ticker_read();
    while (us_ticker_read() - tick_us < timeout_ms * 1000) {
        if (stream->ready()) {
            const int result = stream->gets(buf, size);
...
const char *Player::last_md5() const
{
    return md5_str;
}

uint8_t *Player::scratch_buffer()
{
    return fbuff;
}

size_t Player::scratch_size() const
{
    return sizeof(fbuff);
}

int Player::decompress(const std::string& sfilename, const std::string& dfilename, uint32_t sfilesize, StreamOutput* stream)
{
    FILE *f_in = fwfs::fopen(sfilename.c_str(), "rb");
    FILE *f_out = fwfs::fopen(dfilename.c_str(), "w+");
    char message[96] = {0};
    bool success = false;

    if (sfilesize < 2) {
        snprintf(message, sizeof(message), "Error: compressed file is too small!\r\n");
    } else if (f_in == nullptr || f_out == nullptr) {
        snprintf(message, sizeof(message), "Error: failed to create file [%s]!\r\n", dfilename.substr(0, 30).c_str());
    } else {
        uint16_t checksum = 0;
        uint32_t block_size = 0;
        uint32_t block_count = 0;
        uint32_t idle_count = 0;
        uint8_t header[BLOCK_HEADER_SIZE] = {0};
        qlz_state_decompress decompress_state;
        uint8_t *transfer_buffer = comms::TransferBuffer::data();

        success = true;
        for (uint32_t offset = 0; success && offset < sfilesize - 2; offset += BLOCK_HEADER_SIZE + block_size) {
            if (fwfs::fread(header, sizeof(char), BLOCK_HEADER_SIZE, f_in) != BLOCK_HEADER_SIZE) {
                snprintf(message, sizeof(message), "Error: failed to read compressed block header!\r\n");
                success = false;
                break;
            }

            block_size = (static_cast<uint32_t>(header[0]) << 24)
                       | (static_cast<uint32_t>(header[1]) << 16)
                       | (static_cast<uint32_t>(header[2]) << 8)
                       | static_cast<uint32_t>(header[3]);
            if (block_size < 3 || block_size > comms::TransferBuffer::capacity()) {
                snprintf(message, sizeof(message), "Error: invalid compressed block size!\r\n");
                success = false;
                break;
            }

            if (fwfs::fread(transfer_buffer, sizeof(char), block_size, f_in) != block_size) {
                snprintf(message, sizeof(message), "Error: failed to read compressed block!\r\n");
                success = false;
                break;
            }

            const char *compressed_block = reinterpret_cast<const char *>(transfer_buffer);
            const uint32_t compressed_size = qlz_size_compressed(compressed_block);
            if (compressed_size != block_size) {
                snprintf(message, sizeof(message), "Error: compressed block size mismatch!\r\n");
                success = false;
                break;
            }

            const uint32_t expected_decompressed_size = qlz_size_decompressed(compressed_block);
            if (expected_decompressed_size == 0 || expected_decompressed_size > sizeof(fbuff)) {
                snprintf(message, sizeof(message), "Error: invalid decompressed block size!\r\n");
                success = false;
                break;
            }

            const uint32_t decompressed_size = qlz_decompress(compressed_block, fbuff, &decompress_state);
            if (decompressed_size == 0 || decompressed_size != expected_decompressed_size) {
                snprintf(message, sizeof(message), "Error: failed to decompress block!\r\n");
                success = false;
                break;
            }
            ...
            setvbuf(f_out, reinterpret_cast<char *>(&transfer_buffer[4096]), _IOFBF, 4096);
            if (fwfs::fwrite(fbuff, sizeof(char), decompressed_size, f_out) != decompressed_size) {
                snprintf(message, sizeof(message), "Error: failed to write decompressed block!\r\n");
                success = false;
                break;
            }
        ...
        if (success) {
            uint8_t stored_checksum[2] = {0};
            if (fwfs::fread(stored_checksum, sizeof(char), 2, f_in) != 2) {
                snprintf(message, sizeof(message), "Error: failed to read decompressed checksum!\r\n");
                success = false;
            } else if (checksum != static_cast<uint16_t>((stored_checksum[0] << 8) + stored_checksum[1])) {
                snprintf(message, sizeof(message), "Error: decompressed checksum mismatch!\r\n");
                success = false;
            } else {
                snprintf(message, sizeof(message), "#Info: decompart = %lu\r\n", block_count);
            }
        }
    }

    const bool remove_output = !success && f_out != nullptr;
    if (f_in != nullptr) fwfs::fclose(f_in);
    if (f_out != nullptr) fwfs::fclose(f_out);
    if (remove_output) fwfs::remove(dfilename.c_str());
...
{
    stream->protocol().upload_file(parameters, stream, *this);
}
...
{
    stream->protocol().download_file(parameters, stream, *this);
}
