...
Kernel::RealtimeControlResult Kernel::handle_realtime_control(char received, bool keep_alive, bool allow_feed_hold)
{
    ...
    if (received == '?') {
        result.query = true;
        if (keep_alive) set_keep_alive_request(true);
    } else if (received == 'X' - 'A' + 1) {
        result.halt = true;
    } else if (received == 'Y' - 'A' + 1) {
        if (get_internal_stop_request()) {
            set_internal_stop_request(false);
        } else {
            set_stop_request(true);
            set_stop_request_time(us_ticker_read() / 1000);
        }
    } else if (received == 'Z' - 'A' + 1) {
        set_keep_alive_request(true);
    } else if (allow_feed_hold && is_feed_hold_enabled() && received == '!') {
        set_feed_hold(true);
    } else if (allow_feed_hold && is_feed_hold_enabled() && received == '~') {
        set_feed_hold(false);
    } else {
