...
void init() {
    ...
    // Count this boot attempt as early as possible; the counter is cleared in
    // main() once init() has fully completed. A crash caused by data read
    // during boot recurs identically every boot, so after
    // BootCounter::SAFE_BOOT_THRESHOLD consecutive failures the SD autoloads
    // below are skipped instead of looping forever.
    BootCounter::increment();
    ...
    */
    ...
    if(BootCounter::is_safe_boot()) {
        kernel->streams->printf("SAFE BOOT: %lu consecutive boot attempts failed - skipping SD autoloads "
            "(flex compensation, custom tool slots, config override) this boot\n",
            (unsigned long)(BootCounter::attempts() - 1));
    }
...
*/
    ...
    */
    ...
    if(sdok && !BootCounter::is_safe_boot()) {
...
int main()
{
    ...
    // init() completed without crashing or hanging: clear the boot-attempt
    // counter so the safe-boot threshold only counts consecutive failures.
    BootCounter::mark_boot_ok();
